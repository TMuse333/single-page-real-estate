    ...
    const handleClick = (index:number) => {
      ...
      const dropDownAnimation = (delay:number) => {
       ...
       return (
        ...
        >
          ...
          {src && alt && (
            <Image
              ...
              width={600}
              height={1300}
              className="w-[40vw] h-[60vw]
              max-w-[250px] max-h-[300px] object-contain"
            />
          )}
        ...
        {/* Accordion Section */}
        <div
  className="space-y-4 mt-8 h-[40vw] max-h-[400px] flex flex-col"
>
  {listAspects.map((aspect, index) => (
    <div
      key={index}
      className={`bg-gray-700 rounded-lg overflow-hidden flex-grow transition-all duration-500 ease-in-out ${
    ...
    >
      ...
      <div
        className={`transition-opacity duration-500 ease-in-out ${
          expandedIndex === index ? 'opacity-100' : 'opacity-0 pointer-events-none'
        }`}
      >
        <div className="p-4">
