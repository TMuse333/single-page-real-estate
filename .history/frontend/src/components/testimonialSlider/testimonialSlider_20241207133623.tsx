...
const TestimonialBox: React.FC<{
  ...
  const totalTranslateDistance = (): number => {
    ...
    return vwInPx * isMobile ? const totalTranslateDistance = (): number => {
      const cardWidth = Math.min(Math.max(window.innerWidth * 0.6, 250), 600); // Matches the CSS clamp
      return cardWidth * (lineDistance - 2) + cardWidth * 0.3;
    };
    ; // Multiply by the number of testimonials
  };
