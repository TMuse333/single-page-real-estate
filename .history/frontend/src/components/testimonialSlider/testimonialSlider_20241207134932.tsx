...
const TestimonialBox: React.FC<{
  ...
  const calculateInitialTransform = (index: number): number => {
    const vwInPx = Math.max(window.innerWidth * 0.6, 250); // 60vw in pixels
  ...
  //   const vwInPx = Math.max(window.innerWidth * 0.6, 250); // Calculate 60vw in pixels
  //   return vwInPx * (lineDistance-2)+225; // Multiply by the number of testimonials
  // };

  const totalTranslateDistance = (): number => {
    const vwInPx = Math.max(window.innerWidth * 0.6, 250); // Calculate 60vw with a minimum width
    const buffer = vwInPx * 0.1; // Add a small proportional buffer (10% of card width)
    
    // Dynamic distance calculation for any screen size
    return vwInPx * (lineDistance - 1) + buffer; // Move the card behind the line based on number of testimonials
  };
  ...
  return (
    <motion.div
      ref={ref}
      className={`w-[60vw] min-w-[250px] flex flex-col items-center justify-center text-center py-8 px-4 bg-blue-400 rounded-2xl
      ...
      ref={ref}>
        {testimonials.map((testimonial, index) => (
          <TestimonialBox
            ...
            parentInView={inView}
            index={index}
            lineDistance={testimonials.length}
          />
        ))}
      </div>
