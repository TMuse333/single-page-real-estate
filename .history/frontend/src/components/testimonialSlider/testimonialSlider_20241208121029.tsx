...
const TestimonialBox: React.FC<{
  ...
  const calculateInitialTransform = (index: number): number => {
    const vwInPx = Math.min(Math.max(window.innerWidth * 0.6, 600), 800); // Clamp between 600px and 800px
  ...
  const totalTranslateDistance = (): number => {
    const distance = lineDistance - 2; // Adjusted line distance
    const buffer = 225; // Additional buffer
    const vwInPx = Math.min(Math.max(window.innerWidth * 0.6, 600), 800); // Clamp value between 600px and 800px
    return vwInPx * distance + buffer;
  };
  ...
  useEffect(() => {
    if (parentInView && translateValue <= -1050 && !inView) {
  ...
  return (
    <motion.div
      ref={ref}
      className={`w-[60vw] min-w-[600px] max-w-[675px] flex flex-col items-center justify-center text-center py-8 px-4 bg-blue-400 rounded-2xl
