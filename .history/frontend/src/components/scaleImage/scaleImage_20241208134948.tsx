...
const ScaleImage: React.FC<Props> = ({ src, alt }) => {
  ...
  const scale = useTransform(scrollYProgress, [0, 0.5, 0.7, 1], [!scaleComplete ?0.7 : 1, 1, 1, 0.7],  { ease:easeIn });
    ...
    if(title){
       animate(
        ...
        { opacity: 1, y: 0 },
        {
          ease:'easeIn',
          type: "spring",
          stiffness: 2,  // Lower stiffness for a softer spring
          damping: 5,     // Lower damping for more bounce
          mass: 5,   
          delay:2,
          duration:2  // Lighter mass for faster motion
        }
      );
    }
    ...
    if(description){
       animate(
        ...
        { opacity: 1, y: 0 },
        {
          ease:'easeIn',
          type: "spring",
          stiffness: 80,  // Lower stiffness for a softer spring
          damping: 5,     // Lower damping for more bounce
          mass: 0.5,   
          delay:0,
            // Lighter mass for faster motion
        }
      );
    }
  ...
  return (
    ...
    >
        ...
        sm:text-5xl transition-all md:text-6xl opacity-1 translate-y-[-6rem]">Lorem ipsum dolor sit amet consectetur.
        </h2>
